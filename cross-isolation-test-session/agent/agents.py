...
async def handle_request(agent_instance: Agent, prompt_text: str):
    ...
    current_session_id = os.environ.get("SESSION_ID", str(uuid.uuid4()))
    print(f"[{agent_instance.name}] Initializing session {current_session_id}...")
    print(f"[{agent_instance.name}] Dispatching payload with prompt: {prompt_text}")
...
if __name__ == "__main__":
    ...
    test_prompt = os.environ.get("AGENT_PROMPT", "What language framework should we use to write our data streaming pipelines?")
